		...
		if ($GLOBALS['TSFE']->fe_user->user['uid'] != 0) {   // A frontend user is logged in
			if ($GLOBALS['TSFE']->fe_user->user['uid'] != 0) {   // A frontend user is logged in
				if (isset($TX_REGISTER4CAL_DATA['controller'])) {
					$content = $TX_REGISTER4CAL_DATA['controller']->ListViewRegistration_Submit();
				} elseif (isset($TX_REGISTER4CAL_DATA['error'])) {
					$content = $TX_REGISTER4CAL_DATA['error'];
				}
			}
		}
		return $content;
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/register4cal/calview/class.tx_register4cal_submit.php']) {
	include_once ($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/register4cal/calview/class.tx_register4cal_submit.php']);
}
?>