...
class tx_register4cal_render {
	...
	if (t3lib_div::int_from_ver(TYPO3_version) <= 4003000) {
	    ...
	    $this->event['object'] = &t3lib_div::makeInstance('tx_cal_phpicalendar_model', $event, FALSE, 'tx_cal_phpicalendar');
	}
	...
	if (!isset($conf))
	    return $this->renderError('Configuration "' . $confName . '" not set!');
	...
	if ($template == '')
	    return $this->renderError('Template "' . $templateSubpart . '" not set!');
	if ($templateSubpartSubpart != '')
	    $template = $this->cObj->getSubpart($template, $templateSubpartSubpart);
	...
	$config = $conf[$field . '.'];
	if (!isset($config))
	    $config = $this->settings['default'][$field . '.'];
	...
	// value of field
	if ($mode == 'edit')
	    $value = isset($this->pi_base->piVars[$fieldname]) ? $this->pi_base->piVars[$fieldname] : $field['defaultvalue'];
	...
	switch ($singleMarker) {
		...
		break;
	    case 'NUMATTENDEES':
		if (!isset($this->event['regcount']))
		    $this->event['regcount'] = tx_register4cal_user1::getRegistrationCount($this->event['data']['uid'], $this->event['data']['start_date'], $this->event['data']['pid']);
		...
		if (preg_match('/EVENT_([A-Z0-9_-])*/', $singleMarker)) {
			...
			case 'organizer_email' :
			    ...
			    break;
			case 'formated_start' :
			    if (!isset($this->event['formatedStart']))
				$this->prepareFormatedDateTime();
