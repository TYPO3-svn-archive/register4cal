...
class tx_register4cal_registration_model {
	...
	public function getUserField($field) {
		if (isset($this->user[$field])) return $this->user[$field];
		else return '';
	}
	...
	public function getEventField($field) {
		if (isset($this->event[$field])) return $this->event[$field];
		else return '';
	}
	...
	public function getLocationField($field) {
		if (isset($this->location[$field])) return $this->location[$field];
		else return '';
	}
	...
	public function getOrganizerField($field) {
		if (isset($this->organizer[$field])) return $this->organizer[$field];
		else return '';
	}
	...
	public function getRegistrationField($field) {
		if (isset($this->registration[$field])) return $this->registration[$field];
		else return '';
	}
	...
	public function getUserdefinedField($field) {
		if (isset($this->userdefinedFields[$field])) return $this->userdefinedFields[$field];
		else return FALSE;
	}
	...
	public function setUserdefinedFieldValue($field, $value) {
		if (!isset($this->userdefinedFields[$field])) throw new Exception('Unknown userdefined field "' . $field . '"!');
	...
	public function __construct($eventId=0, $eventDate=0, $userId=0, $registrationId=0) {
		if ($registrationId != 0) {
			...
			if (count($this->registration) == 0) throw new Exception('Unknown Registration "' . $registrationId . '"!');

			$eventId = $this->registration['cal_event_uid'];
			$eventDate = $this->registration['cal_event_getdate'];
			$userId = $this->registration['feuser_uid'];
		}
		if (intval($eventId) == 0) throw new Exception('Missing EventId!');
		if (intval($eventDate) == 0) throw new Exception('Missing EventDate!');
		...
		if (count($this->registration) == 0) $this->registration = $this->readRegistrationRecord();
	...
	public static function getRegistrationsForOrganizer($pidlist) {
		...
		while (($row = $TYPO3_DB->sql_fetch_assoc($result))) {
			...
			if ($registration->userIsOrganizer()) $registrations[$registration->getEventDate()][$registration->getEventField('uid')][] = $registration;
		}
		$TYPO3_DB->sql_free_result($result);
		...
		while (($row = $TYPO3_DB->sql_fetch_assoc($result))) {
			...
			if ($registration->userIsOrganizer()) $registrations[$row['start_date']][$row['uid']][] = $registration;
		}
		$TYPO3_DB->sql_free_result($result);
	...
	public static function getEventsWithWaitlistEntries($eventId=0, $eventDate=0) {
		...
		if ($eventId) $where .= ' AND cal_event_uid=' . intval($eventId);
		if ($eventDate) $where .= ' AND cal_event_getdate=' . intval($eventDate);
	...
	public static function getRegistrationsForEvent($eventId, $eventDate, $status=0) {
		...
		$where = ' tx_register4cal_registrations.cal_event_uid=' . intval($eventId) . ' AND' .
				...
				$TSFE->cObj->enableFields('tx_register4cal_registrations');
		if ($status) $where .= ' AND tx_register4cal_registrations.status=' . intval($status);
		$orderBy = 'crdate ASC';
		$result = $TYPO3_DB->exec_SELECTquery($select, $table, $where, '', $orderBy, '');
		$registrations = Array();
		while (($row = $TYPO3_DB->sql_fetch_assoc($result))) {
	...
	public function register(&$messages) {
		...
		if ($this->numberOfAttendeesUserfield) $numAttendees = intval($this->userdefinedFields[$this->numberOfAttendeesUserfield]['value']);
		if (!$numAttendees) $numAttendees = 1;
		...
		switch ($this->status) {
			...
			case 2:
				...
				break;
			case 3:
				if ($this->event['tx_register4cal_maxattendees'] == 0) $registrationStatus = 1;
				else {
					$numFree = $this->event['tx_register4cal_maxattendees'] - $this->event['tx_register4cal_numregistered'];
					if ($numFree >= $numAttendees) $registrationStatus = 1;
					else {
		...
		if (!$hasErrors) {
			// set data
			if (!$this->status == 3 && !$this->status == 4) throw new Exception('Fatal Error: Wrong call to store registration! (0x02');
			if ($registrationStatus == 2 && $this->settings->disableWaitlist == 1) throw new Exception('Fatal Error: Wrong call to store registration! (0x04');
	...
	public function unregister() {
		if (!$this->status == 5 && !$this->status == 6) throw new Exception('Fatal Error: Wrong call to store registration! (0x03');
		if ($this->settings->disableUnregister != 0) throw new Exception('Fatal Error: Wrong call to store registration! (0x05)');
	...
	public function userIsOrganizer() {
		...
		if (in_Array($feUserId, $this->settings->adminUsers)) return true;

		// is user selected in organizer feUserId field
		if (t3lib_div::inList($this->organizer['tx_register4cal_feUserId'], 'fe_users_' . $feUserId)) return true;
		...
		foreach ($feGroupIds as $feGroupId) {
			if (t3lib_div::inList($this->organizer['tx_register4cal_feUserId'], 'fe_groups_' . $feGroupId)) return true;
		}
		return false;
	}
	...
	public function waitlistCheck() {
		...
		if ($this->registration['status'] != 2) return FALSE;
		if ($this->registration['deleted'] != 0) return FALSE;
		if ($this->event['tx_register4cal_maxattendees'] - $this->event['tx_register4cal_numregistered'] < $this->registration['numattendees'] && $this->registration['numattendees'] != 0) return FALSE;
	...
	private function determineStatus() {
		...
		if ($this->event['tx_register4cal_activate'] != 1) return 0;

		// Status 5/6: User has already registered or enlisted on waitlist
		...
		$start = $this->event['tx_register4cal_regstart'];
		$start = (isset($start) && $start != 0) ? $start : $now;
		$ende = $this->event['tx_register4cal_regend'];
		$ende = (isset($ende) && $ende != 0) ? $ende : strtotime($this->eventDate);
		if ($start > time() || $ende < time()) return 1;
		...
		if ($this->event['tx_register4cal_maxattendees'] != 0) {
			if ($this->event['tx_register4cal_numfree'] == 0) {
				if ($this->event['tx_register4cal_waitlist'] == 1 && $this->settings->disableWaitlist == 0) return 4;
				else return 2;
			}
		}
	...
	private function readEventRecord($eventId, $eventDate) {
		...
		if (t3lib_div::int_from_ver(TYPO3_version) <= 4003000) {
			...
			$object = new $tx_cal_phpicalendar_model($event, FALSE, 'tx_cal_phpicalendar');
		} else {
			$object = &t3lib_div::makeInstance('tx_cal_phpicalendar_model', $event, FALSE, 'tx_cal_phpicalendar');
		}

		// format date and time and append to event
		$start = new tx_cal_date();
		$start->copy($object->getStart());
		$end = new tx_cal_date();
		$end->copy($object->getEnd());
		if ($event['freq'] != 'none') {
			$delta = (strtotime($eventDate) - strtotime($event['start_date']));
			$start->addSeconds($delta);
			$end->addSeconds($delta);
		}
		$format = $event['allday'] == 0 ? $this->settings->dateFormat . ' ' . $this->settings->timeFormat : $this->settings->dateFormat;
		$event['formated_start'] = $start->format($format);
		$event['formated_end'] = $end->format($format);
		if ($event['allday'] != 0) {
			$event['formated_start'] .= ' ###LABEL_event_allday###';
			$event['formated_end'] = '';
		}
		$event['start_date'] = $start->format($this->settings->dateFormat);
		$event['end_date'] = $end->format($this->settings->dateFormat);
		$event['start_time'] = $object->getStart()->format($this->settings->timeFormat);
		$event['end_time'] = $object->getEnd()->format($this->settings->timeFormat);
